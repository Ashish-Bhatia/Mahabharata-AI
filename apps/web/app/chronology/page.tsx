...
export default function ChronologyPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return events.filter((event) =>
      event.names.some((name) => name.text.toLocaleLowerCase().includes(needle)),
    );
  }, [events, query]);
  ...
  return (
    <main style={{ maxWidth: 760, margin: "48px auto", padding: 24 }}>
      ...
      <input
        ...
        placeholder="Kurukshetra or कुरुक्षेत्र"
      ...
      {!loading && !error && filtered.map((event) => (
        <article key={event.id} style={{ borderLeft: "4px solid currentColor", padding: "8px 16px", marginBottom: 20 }}>
          <p><strong>{event.sequence}</strong></p>
          <h2>{eventTitle(event)}</h2>
          <p>{event.description}</p>
          <p><strong>Participants:</strong> {event.character_ids.join(", ")}</p>
          <p><strong>Source:</strong> {event.source_refs.join(", ")}</p>
        </article>
      ))}
      {!loading && !error && filtered.length === 0 && <p role="status">No matching event found.</p>}
    </main>
  );
}
